...
---
## This markdown contains code for joining sanctions datasets with the sentiment scores obtained from LIWC
```{r}
library(tidyverse)
library(readxl)
library(writexl)
...
library(lmtest)
# Reading in the data 
Sanctions <- read_excel("GSDB_V2 (1)/GSDB_V2.xls")
US_aid <- read_csv("us_foreign_aid_country.csv")
WSJ_LIWC <- read_csv("full years/WSJ_5_LIWC.csv")
...
## Fixed effects (using a different dataset)

```{r}
data <- read_csv("fixed.csv") %>%
  mutate(across(
    c(foreign_aid, population, GDP, education, UN_distance, 
      geographic_distance, trade_flow, sentiment),
    ~ log(. + 1)  # Add 1 to handle zeroes before logging
  ))


formula <- foreign_aid ~ population + GDP + education + UN_distance + 
  geographic_distance + trade_flow + war + democracy + sentiment

# Define the panel data structure
pdata <- pdata.frame(data, index = c("country_id", "year"))

# Fixed effects model with two-way fixed effects
model <- plm(
  formula,
  data = pdata,
  model = "within",
...
summary(model, vcov = vcovHC(model, type = "HC0"))

```



## Building an alternative model (logistic regression)
