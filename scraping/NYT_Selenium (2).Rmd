...
```{r}
nyt_2014 <- read_csv('~/Documents/All things R /CURI /NYT_related /Metadata only/nyt_2014_metadata') %>%
  mutate(full_text = NA)
```
...
```{r}
nyt_2014_pa <- read_csv('~/Documents/All things R /CURI /NYT_related /Metadata only/PA /nyt_2014_pa.csv')
...
true_nyt_scraper <- function(dataset, start_date, final_date) {
  ...
  dataset
  write_csv(dataset, '~/Documents/All things R /CURI /NYT_related /Metadata only/PA /nyt_2014_pa.csv')
...
mega_scraper <- function (dataset, start_date, final_date){
  ...
  dataset_full
  write_csv(dataset_full, "~/Documents/All things R /CURI /NYT_related /Metadata only/PA /nyt_2014_pa.csv") ## Change each year 
}

nyt_2014_pa <- mega_scraper(nyt_2014_pa, "2014-01-03", "2014-01-05") ## Change the name each year and make sure it is pa in the brackets when you run the second time  
...
```{r}
write_csv(nyt_2014_pa, "~/Documents/All things R /CURI /NYT_related /Metadata only/PA /nyt_2008_pa.csv")
```
